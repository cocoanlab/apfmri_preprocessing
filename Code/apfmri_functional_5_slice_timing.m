function PREPROC = apfmri_functional_5_slice_timing(subject_dir, session_num, varargin)
...
input_slicetime = false;
mbf = 1; % if not multi-band, mbf = 1;

for i = 1:numel(varargin)
    if ischar(varargin{i})
        switch varargin{i}
            case {'TR', 'tr'} % in seconds
                tr = varargin{i+1};
            case {'multiband', 'MBF', 'MB'}
                mbf = varargin{i+1}; % multiband factor
            case {'acquisition', 'acq'}
                acq = varargin{i+1}; % acquisition 
                % currently works only for interleaved_TD (top-down) and 
                % interleaved_BU (bottom-up).
            case {'slice_time', 'slicetime'}
                input_slicetime = true;
                slice_time = varargin{i+1};
                if size(slice_time,1)>size(slice_time,2), slice_time = slice_time'; end
        end
    end
end
